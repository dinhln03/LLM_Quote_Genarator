...
class QuoteTranslator:
    ...
    def get_quote(self, quotes, suggest_topics, batch_index):
        ...
        *Parameters:
        ...
        prompt = self.prompt_template.format(
            ...
            QUOTE_LIST=[{'original_quote': x, 'suggested_topic': y} for x, y in zip(quotes, suggest_topics)]
        )
        ...
        while True:
            try:
                ...
                logger.error(f"Batch {batch_index}: Resource exhausted. Retrying in 5s...")
                time.sleep(5)
    ...
    def process_batch(self, batch_quote, batch_index):
        ...
        *Parameters:
        ...
        response = self.get_quote(batch_quote["quote"], batch_quote["category"], batch_index)
        if response:
            try:
                ...
                logger.error(f"Batch {batch_index}: Error in parsing response. Retrying...")
    ...
    def translate_and_save_data(self, start_point, end_point, quote_sample, num_threads):
        '''
        ...
        *Parameters:
        ...
        assert total_batches*5 <= len(quote_sample), "Length of quote sample must be greater than the number of batches."
        assert num_threads <= total_batches, "Number of threads must be less than the number of batches." 

        logger.info("Start translating quotes in multi-threading mode with batch size 5...")

        if total_batches % num_threads != 0:
            left_batches = total_batches % num_threads
            end_point -= left_batches  
            logger.warning(f"Reseting end point to {end_point} to make the number of batches divisible by the number of threads.")

        for i in range(start_point, end_point+1, num_threads):
        
            logger.info(f"Processing batch {i}-->{i+num_threads-1} in multi-threading...")
            with concurrent.futures.ThreadPoolExecutor() as executor:
                responses = executor.map(
                    self.process_batch,
                    [quote_sample[(i+j)*batch_size:((i+j)+1)*batch_size] for j in range(num_threads)],
    ...
    def save_results(self, start_index, num_threads, results):
        ...
        with open(f"../data/translated_quote_batch_{start_index}_{start_index+num_threads-1}.json", "w") as f:
            json.dump(results, f, indent=2)
        logger.success(f"Batch {start_index}-->{start_index+num_threads-1} saved successfully.")
