...
def load_model(model_name_or_path, max_seq_length):
    tokenizer = AutoTokenizer.from_pretrained(
        model_name_or_path, use_fast=True, padding_side="right"
    )
    try:
    ...
    # Typing weight
    model.lm_head.modules_to_save.default.weight = (
...
def load_dataset(
    tokenizer: AutoTokenizer, dataset_name_or_path: str, max_seq_length: int
):
    ...
    def convert_conversation_to_input(examples):
        ...
        for quote, topic in zip(examples["quote"], examples["topic"]):
            ...
            input_id = tokenizer.apply_chat_template(example, tokenize=True)[
                :max_seq_length
            ]
