...
from .kernels import QUANT_STATE, fast_dequantize, get_lora_parameters
...
def unsloth_save_model(
    ...
    if push_to_hub and hasattr(model, "config"):
        print(
            f"Saved merged model to https://huggingface.co/{username}/{save_directory.lstrip('/')}"
        )
    pass
        ...
        with subprocess.Popen(
            ...
            for line in sp.stdout:
                ...
                if "undefined reference" in line:
                    raise RuntimeError(
                        "Failed compiling llama.cpp. Please report this ASAP!"
                    )
...
def _fix_gemma_gguf():
    ...
    bad_text = b"""         data = data.astype(np.float32)

            # if f16 desired, convert any float32 2-dim weight tensors to float16
            if self.ftype == 1 and data_dtype == np.float32 and name.endswith(".weight") and n_dims == 2:
                data = data.astype(np.float16)"""
    good_text = b"""         # if f32 desired, convert any float16 to float32
    ...
    gemma_text = (
        gemma_text[:find_bad] + good_text + gemma_text[find_bad + len(bad_text) :]
    )
...
def save_to_gguf(
    ...
    if quantization_method == "f32":
        first_conversion = "f32"
    elif quantization_method == "f16":
        first_conversion = "f16"
    elif quantization_method == "q8_0":
    ...
    if use_fast_convert:
        ...
        command = (
            f"python llama.cpp/convert-hf-to-gguf.py {model_directory} "
    ...
    with subprocess.Popen(
        command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1
    ) as sp:
        for line in sp.stdout:
            line = line.decode("utf-8", errors="replace")
            if "undefined reference" in line:
    ...
    if quantization_method != first_conversion:
        ...
        command = (
            f"./llama.cpp/quantize {old_location} "
